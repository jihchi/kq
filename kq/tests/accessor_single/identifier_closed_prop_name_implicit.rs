...
fn absent_equal() {
    Command::cargo_bin("kq")
        .unwrap()
        .arg(r#"does_not_exist[uses = "actions/checkout@v1"]"#)
        .write_stdin(indoc! {r#"
            step uses="actions/checkout@v1"
            step run="cargo test --all --verbose"
            step uses="actions-rs/toolchain@v1"
...
fn absent_starts_with() {
    Command::cargo_bin("kq")
        .unwrap()
        .arg(r#"does_not_exist[uses ^= "actions"]"#)
        .write_stdin(indoc! {r#"
            step uses="actions/checkout@v1"
            step run="cargo test --all --verbose"
            step uses="actions-rs/toolchain@v1"
